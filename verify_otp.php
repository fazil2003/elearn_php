...
include_once('database_connectivity.php');
?>
...
if (isset($_POST['upload'])) {
	...
	if(!empty($_POST['otp'])){
		...
		if(mysqli_num_rows($result)>0){
			...
			mysqli_query($db,"INSERT INTO users (name,user,password,email) VALUES ('$username_capital','$username','$password','$email')");
			
	
			$result=mysqli_query($db,"SELECT * FROM users WHERE user='$username'");
			...
			while($row=mysqli_fetch_array($result)){
				if($row['user']==$username){
					...
					$user_id=$row['id'];
					mysqli_query($db,"INSERT INTO completion(user_id) VALUES ('$user_id')");
